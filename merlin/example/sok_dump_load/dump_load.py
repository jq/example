...
if __name__ == "__main__":
    ...
    for optimizer_id, optimizer in enumerate(optimizers):
        ...
        # Third , assign all the sok_var to zero , like a memset , so we can make sure load is vaild
        for sok_var in sok_vars:
            ex_indices, ex_values = sok.export(sok_var)
        ...
        for i in range(len(sok_vars)):
            ...
            var_pos = np.searchsorted(sok_var_index_nps_raw[i][var_sorted], sok_var_index_nps_new[i])
        ...
        if have_state:
            for i, tmp_slot_states_list in enumerate(slot_states_list_new):
                for j, _ in enumerate(tmp_slot_states_list):
        ...
        print("[SOK INFO] dump load distribute dynamic test %dth optimizer successfully" % optimizer_id)
